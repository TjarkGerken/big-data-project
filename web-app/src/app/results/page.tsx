...
const Page = () => {
    ...
    async function getTrackData(trackId:string) {
        const response = await axios.post(`/api/get-track-data`, { track_name: trackId }, { headers: { Authorization: localStorage.getItem("authCode") } });
    ...
    return (
        <div className="flex flex-col items-center">
            <h1 className="text-2xl font-bold mt-10 mb-4 bg-spotify-green rounded-full box-border">Your favourite Artist</h1>
            {favoriteArtist && <p className="text-xl mt-5">{favoriteArtist.artist_name}</p>}
            <h1 className="text-2xl font-bold mt-80 mb-4 bg-spotify-green rounded-full">Your most listened track</h1>
            {favoriteTrack && <p className="text-xl mt-5">{favoriteTrack.track_name}</p>}
            <button onClick={getTrackData('Ich will es bar')} className="bg-spotify-green text-white font-bold rounded-full mt-10 px-5 py-2">Get my data</button>
        </div>
    );
};
